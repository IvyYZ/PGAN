...
class CustomPoseGenerator(nn.Module):
    def __init__(self, pose_feature_nc, reid_feature_nc, noise_nc, pose_nc=18, output_nc=3, 
        ...
        en_avg = [nn.LeakyReLU(0.2, True),
                nn.Conv2d(ngf * 8, pose_feature_nc,
                    kernel_size=(12,4), bias=self.use_bias),
        ...
        # N*512*8*4  batch*channels*height*width
        ...
        # N*3*256*128
        #----------------------------------------------------------------
        self.up_sam5 =self._upsample2x_decode(ngf * input_channel[connect_layers][0],ngf * 8)
        self.up_sam4 =self._upsample2x_decode(ngf * input_channel[connect_layers][1],ngf * 4)        
        self.up_sam3 =self._upsample2x_decode(ngf * input_channel[connect_layers][2],ngf * 2)
        self.up_sam2 =self._upsample2x_decode(ngf * input_channel[connect_layers][3],ngf) 
        #self.up_sam1 =self._upsample2x_decode(ngf * input_channel[connect_layers][3],3) 
        
        self.de_fuse5 =self._fuse_decode (ngf * 8,ngf * 8)
        self.de_fuse4 =self._fuse_decode (ngf * 4,ngf * 4)
        self.de_fuse3 =self._fuse_decode (ngf * 2,ngf * 2)
        self.de_fuse2 =self._fuse_decode (ngf,ngf)
    ...
    def _make_layer_decode(self, in_nc, out_nc):
        ...
        return nn.Sequential(*block)  #,dilation=2

#-----------------------------------------------------------------------------
    def _upsample2x_decode(self, in_nc, out_nc):
        upsample2x = [nn.ReLU(False),
                nn.Conv2d(in_nc, out_nc,
        ...
        ]      #nn.Upsample(scale_factor=2, mode='nearest') nn.MaxUnpool2d(4,stride=2, padding=1,True)     
        return nn.Sequential(*upsample2x)

    def _fuse_decode(self, in_nc, out_nc):    
        de_fuse = [nn.ReLU(False),
                    nn.Conv2d(in_nc ,out_nc,
                        kernel_size=(1,1), bias=self.use_bias),
                    self.norm_layer(out_nc),
                    nn.Dropout(self.dropout)]
        return nn.Sequential(*de_fuse)
#--------------------------------------------------------------------------------
    ...
    def decode2(self, model, fake_feature, cnlayers):
        if cnlayers>0:
            return model(fake_feature,pose_feature), cnlayers-1
        else:
            return model(fake_feature), cnlayers
...
#        fake_feature_1, cnlayers = self.decode(self.de_conv1, fake_feature_2, pose_feature_1, cnlayers)
        ...
        #------------------------------------------------------------------------------------------ 
        #pdb.set_trace()    
        #fake_feature_0, cnlayers =self.decode(nn.MaxPool2d(91,1), fake_feature_1, pose_feature_1, cnlayers)
        #------------------------------------------------------------------------------------------               
        fake_feature_5, cnlayers = self.decode(self.de_conv5, fake_feature, pose_feature_5, cnlayers)
        up_fake_5, cnlayers = self.decode2(self.up_sam5, fake_feature, cnlayers)        
        #fake_addfeature_5 =self.de_fuse5(fake_feature_5+up_fake_5)
        fake_addfeature_5 =fake_feature_5+up_fake_5
        
        fake_feature_4, cnlayers = self.decode(self.de_conv4, fake_addfeature_5, pose_feature_4, cnlayers)
        up_fake_4,cnlayers=self.decode2(self.up_sam4,fake_feature_5,cnlayers)
        #fake_addfeature_4 = self.de_fuse4(fake_feature_4+up_fake_4)        
        fake_addfeature_4 = self.de_fuse4(fake_feature_4+up_fake_4) 
        
        fake_feature_3, cnlayers = self.decode(self.de_conv3, fake_addfeature_4, pose_feature_3, cnlayers)
        up_fake_3,cnlayers=self.decode2(self.up_sam3,fake_feature_4,cnlayers)
        fake_addfeature_3 =fake_feature_3+up_fake_3
        #fake_addfeature_3 =self.de_fuse3(fake_feature_3+up_fake_3)
        
        fake_feature_2, cnlayers = self.decode(self.de_conv2, fake_addfeature_3, pose_feature_2, cnlayers)
        up_fake_2,cnlayers=self.decode2(self.up_sam2,fake_feature_3,cnlayers)
        fake_addfeature_2 =fake_feature_2+up_fake_2
        #fake_addfeature_2 =self.de_fuse2(fake_feature_2+up_fake_2)
        
        fake_feature_1, cnlayers = self.decode(self.de_conv1, fake_addfeature_2, pose_feature_1, cnlayers)
        #up_fake_1,cnlayers=self.decode2(self.up_sam1,fake_feature_2,cnlayers)
        #------------------------------------------------------------------------------------------       
        #fake_imgs=fake_feature_5*16+fake_feature_4*8+fake_feature_3*4+fake_feature_2*2+fake_feature_1
